...
export const FillBarChart = ({ percent = '30%', color = '#ffffff', title = 'Title', limit = 10 }) => {
  ...
  return (
    <div style={{ width: '100%', padding: '10px 10px', boxSizing: 'border-box', position: 'relative' }}>
        <span style={{ position: 'absolute', top: -1, left: 18, fontSize: 10, letterSpacing: 0.5, color: theme.text.tertiary }}>{title}</span>
        <span style={{ position: 'absolute', top: -1, right: 16, fontSize: 10, letterSpacing: 0.5, color: theme.text.tertiary }}>{limit}</span>
