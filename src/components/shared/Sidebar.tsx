      ...
      >
        {sideNav.map((nav, index) => {
          ...
          return <div key={index} className='link'>
            <NavLink key={index} to={nav.route} style={{ width: 'calc(100% - 22px)', paddingRight: 7, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxSizing: 'border-box' }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                {nav.icon}
                <span>{language?.[nav.title] || nav.title}</span>
              </div>
              {expandTabs.includes(nav.title) ? <ExpandLessRoundedIcon className='toggle' onClick={(event) => handleExpandLessTabs(event, nav.title)} /> : <ExpandMoreRoundedIcon className='toggle' onClick={(event) => handleExpandMoreTabs(event, nav.title)} />}
            </NavLink>
            {expandTabs.includes(nav.title) && <div style={{ paddingLeft: 10, boxSizing: 'border-box' }}>
              {
